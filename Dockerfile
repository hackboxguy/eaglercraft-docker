...
# Migrated from Eaglercraft 1.5.2 to EaglercraftX 1.8.8
...
# Different repos may have different directory structures, so we search for key files
RUN echo "=== Locating web client files ===" && \
    mkdir -p /build/web && \
    # Find classes.js (the main client JS file - required)
    ...
    # Create index.html if not present (some repos may not include it)
...
RUN cat > $EAGLER_HOME/spigot/server.properties << 'EOF'
server-port=25565
server-ip=0.0.0.0
online-mode=false
spawn-protection=0
...
RUN cat > $EAGLER_HOME/spigot/spigot.yml << 'EOF'
settings:
  bungeecord: true
  timeout-time: 60
...
RUN cat > $EAGLER_HOME/bungee/config.yml << 'EOF'
player_limit: -1
ip_forward: true
permissions:
  default:
...
RUN cat > $EAGLER_HOME/bungee/plugins/EaglerXServer/listeners.yml << 'EOF'
listener_list:
...
RUN cat > $EAGLER_HOME/bungee/plugins/EaglerXServer/settings.yml << 'EOF'
...
RUN cat > $EAGLER_HOME/bungee/plugins/EaglerWeb/settings.json << 'EOF'
{
...
# Create world directory symlink so PandaSpigot worlds persist in the volume
...
RUN cat > /etc/supervisor/conf.d/eaglercraft.conf << 'EOF'
[supervisord]
nodaemon=true
user=root
...
RUN cat > $EAGLER_HOME/scripts/start-services.sh << 'SCRIPT'
#!/bin/bash
set -e
...
# Ensure world directories exist for symlinks
...
# Wait for PandaSpigot to be ready
...
# Start BungeeCord (with EaglerXServer and EaglerWeb)
...
RUN cat > $EAGLER_HOME/scripts/health-check.sh << 'SCRIPT'
#!/bin/bash
# Health check: verify BungeeCord (8081) and PandaSpigot (25565) are listening
